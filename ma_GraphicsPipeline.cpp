...
void GraphicsPipeline::createGraphicsPipeline(const char* vertSpv, const char* fragSpv, VkRenderPass renderPass) {
    std::vector<char> vertShaderCode;
    std::vector<char> fragShaderCode;
    if(!UtilityFunctions::readFile(vertSpv, vertShaderCode)) {
        throw std::runtime_error("Vertex shader file not found");
    }
    if(!UtilityFunctions::readFile(fragSpv, fragShaderCode)) {
    ...
    if (vkCreateGraphicsPipelines(logDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS) {
