...
unsigned char Mineanarchy::TerrainGenerator::sampleVoxel(unsigned int x, unsigned int y, unsigned int z) const {
    ...
    heightMap[heightMapX + heightMapZ*visibleChunkGrid->getChunkSize()*visibleChunkGrid->getGridHalfSideLength()*2] = (unsigned int)(noise.GetNoise((float)x*.1f, (float)z*.1f) * SEA_LEVEL);
