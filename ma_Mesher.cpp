                ...
                if(voxels[ConvertToVoxelMapRelativeIndex(voxelX, voxelY, voxelZ)]) {
                    ...
                    auto getIndex = [&](const vec3& vertex) -> int {
                        ...
                        if (vertexMap.find(vertex) != vertexMap.end()) {
                            return vertexMap[vertex]; // Safely access the value
                        }
                    ...
                    if(faceVisible) {
                        if(getIndex(vec3(baseX, baseY, baseZ)) == -1) {
                            ...
                            vertexMap[vec3(baseX, baseY, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                            //vertexMap[vec3(baseX, baseY, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        if(getIndex(vec3(baseX + 1, baseY + 1, baseZ)) == -1) {
                            chunkLocalVertices.push_back(glm::vec3(baseX + 1, baseY + 1, baseZ));
                            vertexMap[vec3(baseX + 1, baseY + 1, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                            //vertexMap[vec3(baseX + 1, baseY + 1, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        if(getIndex(vec3(baseX, baseY + 1, baseZ)) == -1) {
                            chunkLocalVertices.push_back(glm::vec3(baseX, baseY + 1, baseZ));
                            vertexMap[vec3(baseX, baseY + 1, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                            //vertexMap[vec3(baseX, baseY + 1, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        if(getIndex(vec3(baseX, baseY, baseZ)) == -1) {
                        ...
                        if(getIndex(vec3(baseX+1, baseY, baseZ)) == -1) {
                            ...
                            vertexMap[vec3(baseX+1, baseY, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                            //vertexMap[vec3(baseX+1, baseY, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        if(getIndex(vec3(baseX+1, baseY+1, baseZ)) == -1) {
                            chunkLocalVertices.push_back(glm::vec3(baseX+1, baseY+1, baseZ));
                        ...
                        //if(getIndex(vec3(baseX + 1, baseY, baseZ)) == -1) {
                            ...
                            //vertexMap.insert(accessor, vec3(baseX + 1, baseY, baseZ)); accessor->second = static_cast<unsigned int>(vertices.size() - 1);
                        //}
                        // Front face
                    ...
                    if(faceVisible) {
                        ...
                        if(getIndex(vec3(baseX + 1, baseY + 1, baseZ+1)) == -1) {
                            ...
                            vertexMap[vec3(baseX + 1, baseY + 1, baseZ+1)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        if(getIndex(vec3(baseX, baseY + 1, baseZ+1)) == -1) {
                            chunkLocalVertices.push_back(glm::vec3(baseX, baseY + 1, baseZ+1));
                            vertexMap[vec3(baseX, baseY + 1, baseZ+1)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        if(getIndex(vec3(baseX + 1, baseY+1, baseZ)) == -1) {
                            chunkLocalVertices.push_back(glm::vec3(baseX + 1, baseY+1, baseZ));
                            vertexMap[vec3(baseX + 1, baseY+1, baseZ)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                    ...
                    if(faceVisible) {
                        ...
                        if(getIndex(vec3(baseX+1, baseY, baseZ+1)) == -1) {
                            ...
                            vertexMap[vec3(baseX+1, baseY, baseZ+1)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        if(getIndex(vec3(baseX, baseY, baseZ+1)) == -1) {
                            chunkLocalVertices.push_back(glm::vec3(baseX, baseY, baseZ+1));
                            vertexMap[vec3(baseX, baseY, baseZ+1)] = static_cast<unsigned int>(chunkLocalVertices.size() - 1);
                        }
                        // Bottom face
                        chunkLocalIndices.push_back(getIndex(vec3(baseX, baseY, baseZ)));
                        chunkLocalIndices.push_back(getIndex(vec3(baseX + 1, baseY, baseZ)));
        ...
        for(unsigned int zi = 0; zi < gridHalfSideLength*2; zi++) {
            for(unsigned int xi = 0; xi < gridHalfSideLength*2; xi++) {
                ...
                for(unsigned int i = 0; i < chunkLocalVertices[index].size(); i++) {
                    if(getIndex(vec3(chunkLocalVertices[index][i].pos.x, chunkLocalVertices[index][i].pos.y, chunkLocalVertices[index][i].pos.z)) == -1) {
                        ...
                        vertexMap.insert(accessor, vec3(chunkLocalVertices[index][i].pos.x, chunkLocalVertices[index][i].pos.y, chunkLocalVertices[index][i].pos.z)); // Insert the entry into the map
                ...
                for(unsigned int i = 0; i < chunkLocalIndices[index].size(); i++) {
                    ...
                    if(getIndex(vec3(vertex.pos.x, vertex.pos.y, vertex.pos.z)) != -1) {
                        indices.push_back(accessor->second);
                    }
