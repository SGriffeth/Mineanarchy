...
void Mineanarchy::VisibleChunkGrid::UpdateGridPosition(unsigned int x, unsigned int y, unsigned int z) {
    ...
    memcpy(previousVoxelMap, voxelMap, gridHalfSideLength*2 * chunkSize * gridHalfSideLength*2 * chunkSize * gridHalfSideLength*2 * chunkSize);
...
const std::unordered_map<Mineanarchy::VisibleChunkGrid::vec3, unsigned int, Mineanarchy::VisibleChunkGrid::Vec3Hasher>& Mineanarchy::VisibleChunkGrid::GetChangedVoxels() const {
    return changedVoxels;
}

void Mineanarchy::VisibleChunkGrid::GetPrevGridPosition(unsigned int& x, unsigned int& y, unsigned int& z) const {
    x = this->x;
    y = this->y;
    z = this->z;
}
...
void Mineanarchy::VisibleChunkGrid::UpdateVisibleChunk(unsigned int x, unsigned int y, unsigned int z) {
    unsigned int chunkVoxelIndex = GetVoxelMapIndex(x, y, z);
    unsigned int previousVoxelIndex = GetVoxelMapIndex(this->x - previousx + x, this->y - previousy + y, this->z - previousz + z);
    ...
    if(x < previousx + gridHalfSideLength && x > previousx - gridHalfSideLength) {
        if(y < previousy + gridHalfSideLength && y > previousy - gridHalfSideLength) {
            if(z < previousz + gridHalfSideLength && z > previousz - gridHalfSideLength) {
                // This chunk was previously loaded
                previouslyLoaded = 1;
            }
        }
    }*/
    ...
    for(unsigned int yi = 0; yi < chunkSize; yi++) {
        for(unsigned int zi = 0; zi < chunkSize; zi++) {
            for(unsigned int xi = 0; xi < chunkSize; xi++) {
                ...
                voxelMap[chunkVoxelIndex + xi + yi*chunkSize*chunkSize + zi*chunkSize] = (currentChunkY*chunkSize + yi < generator.getHeightMap(currentChunkX*chunkSize + xi, currentChunkZ*chunkSize + zi) ? 1 : 0);   
                /*if(!previouslyLoaded) {
                    ...
                    voxelMap[chunkVoxelIndex + xi + yi*chunkSize*chunkSize + zi*chunkSize] = previousVoxelMap[previousVoxelIndex + xi + yi*chunkSize*chunkSize + zi*chunkSize];
                }*/
